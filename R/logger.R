...
#' @description Keep track of messages printed by modules or functions
#' @param ...,.envir,.sep passed to \code{\link[glue]{glue}}, if
#' \code{use_glue} is true
#' @param use_glue whether to use \code{\link[glue]{glue}} to combine
...
.logger_functions <- local({
  ...
  colorize <- function(msg, level){
    if(use_crayon()){
      ...
      color <- switch(
        attr(level, "level"),
        FATAL = crayon_ns$combine_styles(crayon_ns$bold, crayon_ns$make_style("#FF0000")),
        ERROR = crayon_ns$make_style("#EE2E00"),
        WARN = crayon_ns$make_style("darkorange"),
        SUCCESS = crayon_ns$combine_styles(crayon_ns$bold, crayon_ns$make_style("green4")),
        INFO = crayon_ns$make_style("#0DCFDA"),
        DEBUG = crayon_ns$make_style("#547783"),
        {
          crayon_ns$make_style("gray50")
        }
      )
      paste0(color(msg), crayon_ns$reset(""))
    } else {
      paste(msg, collapse = " ")
    }
  }
  ...
  set_root_path <- function(root_path, max_bytes, max_files){
    ...
    if(isTRUE(length(root_path) == 1 && root_path != nullfile())){
      if(!(
        ...
        valid_root <- TRUE
        options("rave.logger.root_path" = root_path)
