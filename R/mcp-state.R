...
#'
#' @return An environment of class `"ravepipeline_mcp_session_state"` with
...
#'   \item{`get(key, missing = NULL)`}{Get a value by key. Returns `missing` if key not found}
...
mcptool_state_factory <- function(initialize = NULL,
  ...
  if (force_init || !isTRUE(state_env$.initialized)) {
    if(!inherits(state_env$.stores, "fastmap2")) {
      ...
      class(state_env$.stores) <- c('ravepipeline_fastmap2', 'fastmap2', 'list')
    }
  ...
  class(state_env) <- "ravepipeline_mcp_session_state"
...
#' @export
print.ravepipeline_mcp_session_state <- function(x, ...) {
