...
#'   help topics from (e.g., \code{"ravepipeline"}, \code{"base"}, \code{"utils"}).
...
#' ravepipeline:::mcp_tool_docs_package_help_topics("utils")
#'
...
mcp_tool_docs_package_help_topics <- function(package_name) {
  ...
  if (missing(package_name) || !is.character(package_name) ||
    ...
    return(list(
      success = FALSE,
      error = "package_name parameter is required and must be a non-empty string"
    ))
  }
  ...
  if(system.file(package = package_name) == "") {
    return(list(
      ...
      error = sprintf("Package '%s' is not installed or not available", package_name)
    ))
  }
...
#' res <- ravepipeline:::mcp_tool_docs_help_page("mean", "base")
...
mcp_tool_docs_help_page <- function(topic, package_name = "") {
  ...
  if (!is.character(package_name) || length(package_name) != 1) {
    return(list(
      success = FALSE,
      error = "package_name must be a single character string (use empty string to search all packages)"
    ))
  }
  ...
  if(package_name == "") {
    if(grepl("[:]+", topic)) {
      ...
      if(!length(db$matches)) {
        return(list(
          success = FALSE,
          error = sprintf("unable to find the topic `%s`. Please try <package:function> format or pass package argument explicitly", topic)
        ))
      }
  ...
  if (is.null(rd_obj)) {
    return(list(
      ...
      topic = topic,
      package = package_name,
      error = sprintf("Could not find help documentation for topic '%s' in package '%s'", topic, package_name)
    ))
  }
...
#' \code{ravepipeline-mcp_tool_docs_vignette(package_name, vignette)}.
#'
#' @param package_name Character string, the exact name of the package to list
#'   vignettes from (e.g., \code{"ravepipeline"}, \code{"dplyr"}, \code{"ggplot2"}).
#'   This parameter is required.
#'
#' @return A list containing:
#' \describe{
...
#'   \item{package}{Character, the package name that was queried}
#'   \item{vignettes}{Data frame with columns \code{name} (vignette filename without extension),
#'     \code{title} (vignette title), \code{file} (full filename), and \code{pdf} (PDF path if available)}
...
#'
#' # Example output from calling the tool
#' ravepipeline:::mcp_tool_docs_available_vignettes("data.table")
#'
#' @keywords mcp-tool mcp-category-docs
#' @noRd
...
#'
#' @param package_name Character string, the exact name of the package containing
#'   the vignette (e.g., \code{"ravepipeline"}, \code{"dplyr"}, \code{"ggplot2"}).
#'   This parameter is required.
#' @param vignette Character string, the name of the vignette to retrieve
#'   (without file extension). Use \code{ravepipeline-mcp_tool_docs_available_vignettes}
#'   to see available vignettes. If not specified, defaults to the package name.
...
#'
#' # Example output from calling the tool
#' res <- ravepipeline:::mcp_tool_docs_vignette("data.table", "datatable-faq")
...
mcp_tool_docs_vignette <- function(package_name, vignette = package_name) {
  ...
  if (doc_dir == "") {
    return(list(
      ...
      package = package_name,
      vignette = vignette,
      error = sprintf("No documentation directory found for package '%s'", package_name)
    ))
  }
  ...
  if (is.null(vig_path)) {
    return(list(
      ...
      vignette = vignette,
      error = sprintf(
        "Vignette '%s' not found in package '%s'. Use ravepipeline-mcp_tool_docs_available_vignettes to see available vignettes.",
  ...
  title <- vignette
  title_match <- grep("^%?\\\\VignetteIndexEntry\\{(.*)\\}", content, value = TRUE)
...
#' @param max_results Integer, maximum number of results to return. Default is 20.
#'   Use this to limit output size for broad searches.
...
#'   \item{returned}{Integer, number of results returned (limited by max_results)}
#'   \item{error}{Character, error message if success is FALSE}
#' }
#'
#' @examples
#'
#' # Example output from calling the tool
#' ravepipeline:::mcp_tool_search_packages("ravepipeline")
#'
#' @keywords mcp-tool mcp-category-discovery
#' @noRd
mcp_tool_search_packages <- function(query, max_results = 20) {
  # Validate inputs
  if (missing(query) || !is.character(query) || length(query) != 1 || query == "") {
    return(list(
      success = FALSE,
  ...
  if (!requireNamespace("pkgsearch", quietly = TRUE)) {
    return(list(
      success = FALSE,
      error = "Package 'pkgsearch' is required for searching CRAN packages. Install it with: install.packages('pkgsearch')"
    ))
  }
  ...
  matched_pkgs <- data.frame(
    ...
    description = substr(search_result$description, 1, 200),  # Truncate long descriptions
...
#'   information about (e.g., \code{"ravepipeline"}, \code{"ggplot2"}, \code{"dplyr"}).
#'   This parameter is required.
#'
#' @return A list containing:
#' \describe{
#'   \item{success}{Logical, whether package information was retrieved successfully}
#'   \item{package}{Character, the package name}
#'   \item{info}{List containing package metadata:
#'     \code{version}, \code{title}, \code{description}, \code{author},
#'     \code{maintainer}, \code{license}, \code{depends}, \code{imports},
#'     \code{suggests}, \code{url}, \code{bugreports}, and other DESCRIPTION fields}
#'   \item{error}{Character, error message if success is FALSE}
#' }
#'
#' @examples
#'
#' # Example output from calling the tool
#' res <- ravepipeline:::mcp_tool_search_package_info("ravepipeline")
...
mcp_tool_search_package_info <- function(package_name) {
  ...
  if (is.null(pkg_desc) || inherits(pkg_desc, "try-error")) {
    return(list(
      success = FALSE,
      package = package_name,
      error = sprintf("Could not retrieve information for package '%s'", package_name)
    ))
  }
...
#' - MUST use temporary files for any ephemeral storage needs (\code{tempfile()})
#'
...
#' - AVOID \code{print()} and \code{cat()} unless necessary. If \code{cat()} is unavoidable, you MUST use a SINGLE \code{cat()} call and keep it concise
#' - PREFER returning structured objects (tibbles, data frames, lists) and brief summaries (\code{head()}, \code{str()}, \code{summary()})
...
#'   \item{output}{Character, captured console output (prints, messages, warnings)}
...
#' ravepipeline:::mcp_tool_run_r("mean(1:10)")
#'
#' # Example: Data frame operation
#' ravepipeline:::mcp_tool_run_r("head(mtcars, 3)")
#'
