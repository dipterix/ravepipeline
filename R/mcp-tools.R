...
strip_r_type_prefix <- function(text) {
  ...
  # Pattern: "Type[specifier], rest of description" or "Type[], rest of description"
  # Examples: "Character string, ...", "Logical, ...", "Character[], ..."
  text <- sub("^(Character string|Character vector|Character\\[\\]|Character|Logical|Integer|Numeric|Double|List|Data frame|Matrix|Array\\[\\]|Array|Vector|Function|Environment)([,:]?\\s*)", "", text, ignore.case = TRUE)
...
infer_param_type <- function(param_name, description, func_value = NULL) {
  ...
  # Strategy 0A: Check for Type[] pattern at the start of description (e.g., "Character[], array of...")
  # This should be checked before other patterns
  array_prefix_match <- regexec("^\\s*([A-Za-z]+)\\[\\]", description, ignore.case = TRUE)
  ...
  # Strategy 0B: Extract type from Rd markup (e.g., \\code{character[]})
  # Common patterns: \code{type}, \code{type[]}, \link[pkg]{type}

  # Match \code{type} or \code{type[]} (case-insensitive)
  code_match <- regexec("\\\\code\\{([A-Za-z]+(?:\\[\\])?)\\}", description, ignore.case = TRUE)
  ...
  # First try matching multi-word type patterns like "Character string", "Numeric vector"
  multiword_type_match <- regexec("^\\s*(Character string|Character vector|Numeric vector|Integer vector|Logical vector|Character\\[\\]|Numeric\\[\\]|Integer\\[\\])[,:]?\\s*", description, ignore.case = TRUE)
  ...
  # Extract first word/phrase before period or colon (case-insensitive)
  type_match <- regexec("^\\s*([A-Za-z]+(?:\\[\\])?)[.:]\\s*", description, ignore.case = TRUE)
  ...
  # Check for vector patterns first (e.g., "character vector", "numeric vector")
  vector_patterns <- list(
    string = c("\\bcharacter\\s+vector\\b", "\\bcharacter\\s+array\\b", "\\bstring\\s+vector\\b"),
    number = c("\\bnumeric\\s+vector\\b", "\\bnumeric\\s+array\\b", "\\bdouble\\s+vector\\b"),
    integer = c("\\binteger\\s+vector\\b", "\\binteger\\s+array\\b", "\\bint\\s+vector\\b"),
  ...
  type_patterns <- list(
    ...
    object = c("^\\s*object\\b", "\\blist\\b", "\\benvironment\\b", "\\bdata\\s*frame\\b", "\\bdata\\.frame\\b", "\\btibble\\b"),
...
#' @param param_name Character. Parameter name
#' @param func Function. The R function
#' @param description Character. Parameter description to check if it's singular/plural
...
extract_enum_from_formals <- function(param_name, func, description) {
  ...
  # Check if this is truly an enum (single value selection) vs an array parameter
  # Look for clues in description: "string" suggests single, "array"/"vector" suggests multiple
  desc_lower <- tolower(description)
...
#'
#' @param description Character. Parameter description that may contain example values
...
extract_examples_from_description <- function(description) {
  ...
  # Pattern: "example values", "options are", "valid values", "e.g.", "for example"
  pattern <- "(?i)(example values?|options? (?:are|include)|valid values?|e\\.g\\.|for example|such as)[:\\s]*"
  ...
  for (start_pos in start_positions) {
    ...
    while (pos <= length(chars) && depth > 0) {
      ...
      if (!in_string && (ch == '"' || ch == "'")) {
        in_string <- TRUE
        string_char <- ch
      } else if (in_string && ch == string_char && (pos == 1 || chars[pos - 1] != "\\")) {
        in_string <- FALSE
        string_char <- NULL
      }
...
extract_tool_from_roxygen_block <- function(
  ...
  if (length(desc_start) > 0) {
    ...
    description <- gsub("(^@description\\s+|\\s+)", " ", description) # Normalize whitespace
  ...
  lapply(param_starts, function(param_idx) {
    ...
    while (param_end <= length(clean_lines) && !grepl("^@", clean_lines[param_end])) {
    ...
    if (length(matches) >= 3) {
      ...
      has_default_in_desc <- grepl("default[:\\s]|if\\s+null|optional", param_desc, ignore.case = TRUE)

      # Parameter is required only if it has no default in formals AND no default mentioned in description
  ...
  if (length(return_idx) > 0) {
    ...
    while (return_end <= length(clean_lines) && !grepl("^@", clean_lines[return_end])) {
    ...
    if (grepl("\\\\describe\\{", return_text)) {
      ...
      if (describe_start > 0) {
        ...
        if (describe_end > describe_start) {
          ...
          describe_content <- substring(return_text, describe_start + 10, describe_end - 1)
          ...
          while (pos <= nchar(describe_content)) {
            # Look for \item{
            item_start <- regexpr("\\\\item\\{", substring(describe_content, pos))
            ...
            item_name <- trimws(substring(describe_content, name_start, name_end))
            ...
            if (substring(describe_content, pos, pos) == "{") {
              ...
              item_desc <- trimws(substring(describe_content, desc_start, desc_end))
  ...
  if (length(examples_idx) > 0) {
    ...
    while (examples_end <= length(clean_lines) && !grepl("^@", clean_lines[examples_end])) {
    ...
    if (examples_end > examples_start) {
      ...
      if (length(code_lines) > 0 && !all(startsWith(trimws(code_lines), "#"))) {
        ...
        exec_code <- paste0(
          "res <- { ", code_text, " }\n",
          "ravepipeline::mcp_describe(res, max_print = 5)"
        )
  ...
  # Add class
  class(tool_def) <- c("ravepipeline_mcp_tool", "list")
...
mcptool_build <- function(path = ".", verbose = TRUE) {
  ...
  lapply(r_files, function(r_file) {
    ...
    for (keyword_idx in keyword_lines) {
      ...
      # Collect all roxygen lines from block_start to the line before func_line_idx
      block_lines <- trimws(file_content[seq(block_start, func_line_idx - 1, by = 1L)])
      ...
      tryCatch({
        ...
        # Extract function(...) part - look for function(...) { pattern
        # This handles nested parentheses in default values like c("a", "b")
        func_match <- regexec("function\\s*\\((.*?)\\)\\s*\\{", func_text)
        ...
        if (verbose) {
          warning(
            "Could not parse function formals for '", func_name, "' in ", basename(r_file),
            ". Type inference from defaults will be skipped. Error: ", e$message,
            call. = FALSE
          )
        }
      })
  ...
  for (tool in tools) {
    if (!is.null(tool$name) && nzchar(tool$name)) {
      tool_file <- file.path(tools_dir, sprintf("%s.yaml", gsub("[^a-zA-Z0-9_-]+", "-", tool$name)))
  ...
  if (dir.exists(workflow_dir)) {
    # tryCatch({
      # Load workflows manually (mcpflow_load_all may not be available yet)
      ...
      if (length(yaml_files) > 0) {
        ...
        for (yaml_file in yaml_files) {
          # workflow <- tryCatch({
            ...
            if (!is.list(wf)) {
              warning("Workflow file ", basename(yaml_file), " did not parse as a list", call. = FALSE)
              # NULL
              next
            } else {
              workflow <- wf
          ...
          # }, error = function(e) {
          #   warning("Failed to parse workflow ", basename(yaml_file), ": ", e$message, call. = FALSE)
          #   NULL
          # })
          ...
          if (!is.null(mcp_tools) &&
            ...
            if (length(missing_tools) > 0) {
              warning("Workflow '", workflow_name, "' mcp_tools field references missing tools: ",
                      paste(missing_tools, collapse = ", "),
                      ". Please update workflow YAML.", call. = FALSE)
            }
          }
          ...
          if (!is.null(workflow$jobs) && is.list(workflow$jobs)) {
            ...
            if (length(job_tools) > 0) {
              ...
              if (length(missing_job_tools) > 0) {
                warning("Workflow '", workflow_name, "' jobs reference missing tools: ",
...
#' @param groups Character vector or NULL. Optional group filter(s) to load only
#'   tools from specific group(s). Groups are extracted from tool names following
#'   the pattern \code{pkg-mcp_tool_{group}_{action}}. Each group can be a regexp
#'   pattern. For example, "pipeline", "config", or "(pipeline|config)".
#'   If \code{NULL} (default), all tools are loaded.
...
mcptool_load_all <- function(pkg, groups = NULL) {
  ...
  tools <- lapply(yaml_files, function(f) {
    tryCatch(
      ...
      error = function(e) {
        warning("Failed to load MCP tool from ", basename(f), ": ", e$message, call. = FALSE)
        NULL
      }
    )
  })
...
#'
#' @export
mcptool_read <- function(path, check = FALSE) {
  if (!file.exists(path)) {
  ...
  tool_def <- tryCatch(
    ...
    error = function(e) {
      stop("Failed to parse tool YAML at '", path, "': ", e$message, call. = FALSE)
    }
  )
    ...
    "summary" = {
      ...
      if (!is.null(x$parameters) && !is.null(x$parameters$properties)) {
        ...
        for (param_name in names(x$parameters$properties)) {
          ...
          lines <- c(lines, sprintf("  - %s %s (%s)", param_name, req_marker, type_str))
        }
      }
    ...
    "markdown" = {
      ...
      if (!is.null(x$parameters) && !is.null(x$parameters$properties)) {
        ...
        for (param_name in names(x$parameters$properties)) {
          ...
          req_marker <- if (is_required) "**required**" else "*optional*"
          lines <- c(lines, sprintf("- **%s** (%s, %s)", param_name, type_str, req_marker))
          ...
          if (!is.null(param$enum)) {
            enum_str <- paste(vapply(param$enum, function(e) {
      ...
      if (!is.null(x$returns)) {
        ...
        if (!is.null(x$returns$properties)) {
          ...
          for (prop_name in names(x$returns$properties)) {
            ...
            lines <- c(lines, sprintf("- **%s** (%s)", prop_name, prop_type))
...
mcptool_write <- function(tool, path, method = c("yaml", "markdown"), ...) {
  ...
  if (!inherits(tool, "ravepipeline_mcp_tool")) {
...
#' @param groups Character vector or NULL. Optional group filter(s) to list only
#'   tools from specific group(s). Groups are extracted from tool names following
#'   the pattern \code{pkg-mcp_tool_{group}_{action}}. Each group can be a regexp
...
mcptool_path <- function(tool_name, tools_dir = "../tools") {
  if (!is.character(tool_name) || length(tool_name) != 1 || !nzchar(tool_name)) {
  ...
  if (has_pkg_prefix) {
    ...
    if (nzchar(pkg_path)) {
      ...
      if (file.exists(pkg_tool_path)) {
        tool_def <- mcptool_read(pkg_tool_path)
        if (!is.null(tool_def$name) && tool_def$name == normalized_tool_name) {
          attr(pkg_tool_path, "mcp_definition") <- tool_def
          return(pkg_tool_path)
        } else {
          warning("Tool file '", pkg_tool_path, "' has mismatched name field. Expected '",
                  normalized_tool_name, "', got '", tool_def$name %||% "<missing>", "'")
        }
      }
    }
    ...
    error_msg <- paste0(
      ...
      if (!nzchar(pkg_path)) "package not installed" else
        paste0("tool file not found in package ('", file.path(pkg_path, "mcp", "tools"), "')"),
      "\n  - Local directory lookup: ",
      if (is.null(tools_dir) || !nzchar(tools_dir)) "skipped (no tools_dir specified)" else
        if (!dir.exists(tools_dir)) paste0("directory not found ('", tools_dir, "')")
        else paste0("tool file not found in '", tools_dir, "'")
    )
    stop(error_msg)
    ...
    if (is.null(tools_dir) || !nzchar(tools_dir)) {
      stop("Tool '", tool_name, "' requires tools_dir to be specified (no package prefix)")
    }
