...
`rave_serialize_impl.rave-brain` <- function(object) {
  ...
  # see R/serialize-brain.R
  params <- brain_marshal(object)
...
#' @rdname rave-serialize-refhook
#' @export
`rave_serialize_impl.multi-rave-brain` <- function(object) {
  if (!R6::is.R6(object)) {
    return(NextMethod())
  }
  # "multi-rave-brain" "R6"
  ...
  # see R/serialize-brain.R
  params <- multibrain_marshal(object)
  class(params) <- c("rave_serialized_multi-rave-brain", "rave_serialized")
  rawToChar(serialize(params, NULL, ascii = TRUE))
}
...
#' @rdname rave-serialize-refhook
#' @export
`rave_unserialize_impl.rave_serialized_multi-rave-brain` <- function(x) {
  # see R/serialize-brain.R
  multibrain_unmarshal(x)
}
