...
#'   Default: \code{TRUE}
#'
#' @return An \code{ellmer::ContentText} object containing the formatted context.
#'   This object can be passed directly to \code{chat$chat()}.
#'
#' @details
...
#'   \item Formatted objects are wrapped in code blocks if \code{.wrap_code = TRUE}
...
#' \code{\link{mcpflow_instantiate}} for workflow integration
#'
#' @export
mcp_context <- function(..., .header = "## Context", .wrap_code = TRUE) {
  ...
  formatted <- lapply(args, function(x) {
    ...
    if (is.character(x) && length(x) == 1 && grepl("^\\{[a-zA-Z][a-zA-Z0-9.]*\\}$", x)) {
      ...
      topics <- tryCatch({
        ...
        if (length(help_db$matches) > 0 && nrow(help_db$matches) > 0) {
          ...
          # Format as JSON
          json_str <- jsonlite::toJSON(topic_list, pretty = FALSE, auto_unbox = FALSE)
          paste0(x, "\n```json\n", json_str, "\n```")
        } else {
