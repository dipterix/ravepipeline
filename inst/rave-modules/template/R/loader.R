...
loader_server <- function(input, output, session, ...) {
  ...
  shiny::bindEvent(
    ravedash::safe_observe({
      ...
      default_reference <- isTRUE(loader_reference$get_sub_element_input("default")) # nolint: line_length_linter.

      # --------------------- Run the pipeline! ---------------------

      # Calculate the progress bar
      tarnames <- pipeline$target_table$Names
      count <- length(tarnames) + length(dipsaus::parse_svec(loader_electrodes$current_value)) + 4 # nolint: line_length_linter.
